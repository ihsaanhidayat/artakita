...
export default function ShareWallet({ walletId, onClose, onSuccess, onError }: ShareWalletProps) {
  ...
  const handleShareWallet = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault();
    if (!selectedUid) {
    ...
    try {
      ...
      onError?.("Gagal membagikan dompet: " + (err as Error).message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="text-center mb-6">
        <h3 className="text-base font-black ds-t1 uppercase tracking-widest">Bagikan Akses</h3>

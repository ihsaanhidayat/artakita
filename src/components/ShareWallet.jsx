...
/**
 * ShareWallet — Form berbagi akses dompet ke user lain
 *
 * Props:
 *  - walletId: string
 *  - onClose: () => void
 *  - onSuccess: (message: string) => void  ← dipanggil saat berhasil
 *  - onError: (message: string) => void    ← dipanggil saat gagal
 */
export default function ShareWallet({ walletId, onClose, onSuccess, onError }) {
  ...
  const handleShareWallet = async (e) => {
    ...
    // FIX: ganti alert() → callback prop
    if (!selectedUid) {
      onError?.("Pilih pengguna terlebih dahulu!");
      return;
    }
    ...
    try {
      const { error } = await supabase
        .from("wallet_members")
        .insert({ wallet_id: walletId, user_id: selectedUid, role: "member" });

      if (error) throw error;

      // FIX: ganti alert() → callback prop
      onSuccess?.("Akses dompet berhasil dibagikan! 🎉");
      onClose?.();
    } catch (err) {
      // FIX: ganti alert() → callback prop
      onError?.("Gagal membagikan dompet: " + err.message);
    } finally {
      setIsLoading(false);
    }
  };

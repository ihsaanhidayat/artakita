        ...
        <div className="flex flex-1 bg-gray-100 dark:bg-[#121827] p-2 rounded-[14px] border border-gray-200 dark:border-gray-800/60 shadow-inner">
          {[
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 text-[8px] font-black uppercase tracking-widest py-2 rounded-xl transition-all duration-200 ${activeTab === key
            ...
            >
              {label}
              {count > 0 && (
                <span className={`text-[9px] font-black px-1 py-0.5 rounded-full ${activeTab === key ? "bg-blue-100 dark:bg-white/20 text-blue-600 dark:text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-500"
                  }`}>
                  {count}
                </span>
              )}
          ...
          <>
            ...
            >
              <div className="w-full max-w-sm bg-white dark:bg-[#0d1117] rounded-[24px] shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden"
                ...
                {/* Header */}
                <div className={`px-5 py-3.5 flex items-center justify-between ${formData.type === "debt" ? "bg-red-500/10 border-b border-red-500/20" : "bg-emerald-500/10 border-b border-emerald-500/20"
                  }`}>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex gap-1.5 mt-1">
                      {[
                        { val: "debt", label: DEBT.TAB_DEBT, color: "bg-red-500 text-white", inactive: "text-gray-400" },
                        { val: "receivable", label: DEBT.TAB_RECEIVE, color: "bg-emerald-500 text-white", inactive: "text-gray-400" },
                      ].map(({ val, label, color, inactive }) => (
                        <button key={val} type="button"
                          onClick={() => setFormData(p => ({ ...p, type: val }))}
                          className={`px-2.5 py-1 rounded-lg font-black text-[9px] uppercase tracking-widest transition-all ${formData.type === val ? color : `bg-gray-100 dark:bg-gray-800 ${inactive}`}`}>
                          {label}
                        </button>
                      ))}
                    </div>
                  ...
                  <button
                    ...
                    disabled={isAdding}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest ml-3 shrink-0 transition-all disabled:opacity-40 ${isDirty ? `${formData.type === "debt" ? "bg-red-600" : "bg-emerald-600"} text-white` : "bg-gray-100 dark:bg-gray-800 text-gray-400"
                      }`}
                  >
                    <Save size={11} />
                ...
                <form onSubmit={handleAdd} className="px-5 py-4 space-y-0">
                  ...
                  <div className="border-b border-gray-100 dark:border-gray-800/60 py-3">
                    <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest block mb-1.5">{DEBT.PERSON_NAME}</label>
                    <input type="text" required autoFocus
                      placeholder={DEBT.PERSON_HINT}
                      value={formData.person ?? ""}
                      onChange={e => { setFormData(p => ({ ...p, person: e.target.value })); setIsDirty(true); }}
                      className="w-full bg-transparent outline-none font-bold text-sm text-gray-900 dark:text-white placeholder-gray-300 dark:placeholder-gray-700"
                    />
                  </div>
                  {/* Nominal */}
                  <div className="border-b border-gray-100 dark:border-gray-800/60 py-3">
                    <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest block mb-1.5">{DEBT.NOMINAL}</label>
                    <input type="text" required
                      ...
                      className="w-full bg-transparent outline-none font-black text-2xl text-gray-900 dark:text-white placeholder-gray-300 dark:placeholder-gray-700 border-b-2 border-transparent focus:border-blue-500/50 transition-colors pb-0.5"
                    />
                  </div>
                  {/* Tanggal */}
                  <div className="py-3 flex items-center justify-between">
                    <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Jatuh Tempo</label>
                    <input type="date"

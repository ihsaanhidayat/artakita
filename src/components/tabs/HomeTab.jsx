 ...
 return (
  <div className="relative shrink-0">
   <button ref={btnRef} onClick={handleOpen} disabled={isUploading}
    ...
    style={{
     background: type === "income" ? "color-mix(in srgb, var(--income) 10%, transparent)" : "var(--bg-3)",
     color: type === "income" ? "var(--income)" : "var(--text-2)",
 ...
 const btnStyle = (isActive, key) => ({
  background: isActive
   ? key === "income"
    ...
    : "color-mix(in srgb, var(--a2) 6%, transparent)"
   : "var(--bg-3)",
    ...
    <div className="ds-card p-5 mb-4 ds-fade-1">
     ...
     {(!isOnline || pendingCount > 0) && (
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-[10px] font-bold mt-2"
       style={{ background: !isOnline ? "color-mix(in srgb,#f59e0b 8%,transparent)" : "color-mix(in srgb,#3b82f6 8%,transparent)",
     ...
     <div className="grid grid-cols-2 gap-2.5">
      {[
       ...
       { key: "expense", label: HOME.EXPENSE, val: filteredExpense, color: "var(--text-2)" },
     ...
     {transactions.length === 0 && !mounted ? (
      ...
      return (
       <motion.div key={trx.id} layout
        ...
        style={{
         background: "var(--bg-1)",
         border: `1px solid ${trx._pending ? "color-mix(in srgb,#f59e0b 25%,transparent)" : "var(--border)"}`,
         borderLeft: trx.type === "income" ? `2px solid color-mix(in srgb, var(--income) 30%, transparent)` : `1px solid var(--border)`,
        }}
       >

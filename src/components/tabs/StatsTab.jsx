...
const StatsTab = memo(function StatsTab({ filteredTransactions, transactions, balance, activeWallet }) {
 ...
 <div className="absolute top-0 left-0 right-0 h-0.5 rounded-t-[28px]"
   style={{ background: "linear-gradient(90deg, var(--a1), var(--a3), var(--a2))", opacity: 0.7 }} />

 {/* Grade + message */}
 <div className="flex items-center gap-4 mb-5">
 <div className={`w-14 h-14 rounded-2xl ds-bg-3 flex items-center justify-center text-2xl font-black shrink-0 ${health.color}`}
 ...
 <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{
   background: isOver ? "color-mix(in srgb, var(--a3) 8%, transparent)" : "color-mix(in srgb, var(--income) 8%, transparent)",
   border: `1px solid ${isOver ? "color-mix(in srgb, var(--a3) 20%, transparent)" : "color-mix(in srgb, var(--income) 20%, transparent)"}`,
 }}>
 {isOver
 ...
 const noLim = limit === 0;
 return (
 <div key={cat} className="ds-bg-3 rounded-[18px] p-4"
   style={{ border: isCatOver ? "1px solid color-mix(in srgb, var(--a3) 25%, transparent)" : "1px solid transparent" }}>
 ...
 {!noLim && (
 <span className={`text-xs font-black ${isCatOver ? "ds-expense-glow" : pct >= 80 ? "ds-expense-glow" : "ds-t2"}`}>
 {pct.toFixed(0)}%
 </span>
 )}
 <button

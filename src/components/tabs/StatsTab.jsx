...
const getHealth = (exp, inc) => {
 if (inc === 0 && exp === 0) return { grade: "-", color: "ds-t3", bar: "bg-gray-400", pct: 0, msg: STATS.GRADE["-"] };
 ...
 if (r >= 0.5) return { grade: "B", color: "ds-aurora-text", bar: "bg-blue-500", pct: r*100, msg: STATS.GRADE.B };
...
const StatsTab = memo(function StatsTab({ filteredTransactions, transactions, balance }) {
 ...
 section === key ? "ds-bg-1 ds-aurora-bg ds-aurora-text ds-t1 shadow-sm" : "ds-t3"
 }`}
 >{label}</button>
 ))}
 </div>
 ...
 <div className={`w-14 h-14 rounded-2xl ds-bg-1 flex items-center justify-center text-2xl font-black shrink-0 ${health.color}`}>
 {health.grade}
 </div>
 ...
 <div className="bg-red-500/8 rounded-2xl p-3">
 ...
 <div className="bg-green-500/8 rounded-2xl p-3">
 ...
 >
 {/* Saldo overview */}
 <div className="ds-bg-1 border ds-border rounded-[24px] p-5 shadow-sm">
 <div className="flex justify-between items-start mb-4">
 <div>
 <p className="text-[9px] font-black ds-t3 uppercase tracking-widest mb-1">Saldo Dompet</p>
 <p className="text-2xl font-black ds-t1">Rp {fmt(balance)}</p>
 </div>
 <div className="text-right">
 <p className="text-[9px] font-black ds-t3 uppercase tracking-widest mb-1">{STATS.ALLOCATION}</p>
 <p className={`text-xl font-black ${isOver ? "text-red-500" : "ds-aurora-text"}`}>Rp {fmt(totalAlokasi)}</p>
 </div>
 </div>
 ...
 const noLim = limit === 0;
 return (
 <div key={cat} className={`ds-bg-3 rounded-[18px] p-4 ${isOver ? "border border-red-200 " : ""}`}>
 ...
 {!noLim && (
 <span className={`text-xs font-black ${isOver ? "text-red-500" : pct >= 80 ? "text-red-400" : "ds-t2"}`}>
 {pct.toFixed(0)}%
 </span>
 )}
 <button
 ...
 className={`p-2.5 rounded-xl transition-colors ${editingBudget?.category === cat ? "ds-aurora-text ds-aurora-bg" : "ds-t3 hover:ds-aurora-text ds-bg-1"}`}>
 <Edit3 size={12} />
 </button>
 </div>
 </div>
 ...
 className="flex-1 ds-bg-3 rounded-xl px-3 py-2 text-sm font-bold ds-t1 outline-none focus:ring-1 focus:ring-blue-500 placeholder-gray-400"
 />
 <button
 ...
 isDirtyBudget ? "bg-blue-600 text-white" : "ds-bg-3 ds-t3"
 }`}
 >
 <Save size={10} />
 {isSaving ? "..." : "OK"}

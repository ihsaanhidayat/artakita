...
type StatusKey = "online" | "today" | "week" | "dormant" | "banned";
...
function getStatus(u: AdminUser): StatusKey {
...
type SortKey = "activity" | "transactions" | "name";
...
export default function ActivityMonitor({ onNotify }: ActivityMonitorProps) {
  ...
  const fetchUsers = useCallback(async (silent = false): Promise<void> => {
    ...
    try {
      ...
      onNotify?.("Gagal memuat aktivitas: " + (err as Error).message, "error");
    } finally {
      setIsLoading(false);
    }
  }, [onNotify]);
  ...
  useEffect(() => {
    if (!auto) return;
    const id = setInterval(() => void fetchUsers(true), 30000);
